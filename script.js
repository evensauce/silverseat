        ...
        const VENDORS_STORAGE_KEY = 'silverSeatVendors'; // New key for vendors
        ...
        // --- DOM Elements ---
        const splashScreen = document.getElementById('splash-screen'); const authView = document.getElementById('auth-view'); const customerLoginForm = document.getElementById('customer-login-form'); const customerRegisterForm = document.getElementById('customer-register-form'); const adminLoginForm = document.getElementById('admin-login-form'); const vendorLoginForm = document.getElementById('vendor-login-form'); /* New */ const showRegisterButton = document.getElementById('show-register-button'); const showAdminLoginButton = document.getElementById('show-admin-login-button'); const showVendorLoginButton = document.getElementById('show-vendor-login-button'); /* New */ const backToCustomerLoginButtons = document.querySelectorAll('.back-to-customer-login'); const customerLoginError = document.getElementById('customer-login-error'); const registerError = document.getElementById('register-error'); const adminLoginError = document.getElementById('admin-login-error'); const vendorLoginError = document.getElementById('vendor-login-error'); /* New */ const appHeader = document.getElementById('app-header'); const loggedInUserInfo = document.getElementById('logged-in-user-info'); const logoutButton = document.getElementById('logout-button'); const adminView = document.getElementById('admin-view'); const adminNavButtons = document.querySelectorAll('.admin-nav-button'); const adminManageMoviesSection = document.getElementById('admin-manage-movies'); const adminAnalyticsSection = document.getElementById('admin-analytics'); const adminBookingsSection = document.getElementById('admin-bookings-section'); const adminQrScannerSection = document.getElementById('admin-qr-scanner-section'); const adminManageVendorsSection = document.getElementById('admin-manage-vendors'); /* New */ const adminBookingsListContainer = document.getElementById('admin-bookings-list'); const adminVendorListContainer = document.getElementById('admin-vendor-list'); /* New */ const adminQrReaderElement = document.getElementById('admin-qr-reader'); const startScanBtn = document.getElementById('start-scan-btn'); const scanStatusElement = document.getElementById('scan-status'); const movieForm = document.getElementById('movie-form'); const formTitle = document.getElementById('form-title'); const submitButton = document.getElementById('submit-button'); const cancelEditButton = document.getElementById('cancel-edit-button'); const movieVendorSelect = document.getElementById('movie-vendor'); /* New */ const vendorForm = document.getElementById('vendor-form'); /* New */ const vendorFormTitle = document.getElementById('vendor-form-title'); /* New */ const vendorSubmitButton = document.getElementById('vendor-submit-button'); /* New */ const vendorCancelEditButton = document.getElementById('vendor-cancel-edit-button'); /* New */ const adminMovieListContainer = document.getElementById('admin-movie-list'); const customerPreviewContainer = document.getElementById('customer-movie-list-preview'); const noMoviesAdminMsg = document.getElementById('no-movies-admin'); const noMoviesPreviewMsg = document.getElementById('no-movies-preview'); const analyticsAvgRatingsContainer = document.getElementById('analytics-avg-ratings'); const analyticsOccupancyContainer = document.getElementById('analytics-occupancy'); const customerView = document.getElementById('customer-view'); const customerMovieListContainer = document.getElementById('customer-movie-list'); const noMoviesCustomerMsg = document.getElementById('no-movies-customer'); const toggleMyBookingsBtn = document.getElementById('toggle-my-bookings-btn'); const myBookingsView = document.getElementById('my-bookings-view'); const myBookingsListContainer = document.getElementById('my-bookings-list'); const backToMoviesBtn = document.getElementById('back-to-movies-btn'); const vendorDashboardView = document.getElementById('vendor-dashboard-view'); /* New */ const vendorMovieListContainer = document.getElementById('vendor-movie-list'); /* New */ const noVendorMoviesMsg = document.getElementById('no-vendor-movies'); /* New */ const movieDetailsModal = document.getElementById('movie-details-modal'); const modalCloseButton = document.getElementById('modal-close-button'); const modalMovieTitle = document.getElementById('modal-movie-title'); const modalMoviePoster = document.getElementById('modal-movie-poster'); const modalAvgRatingContainer = document.getElementById('modal-avg-rating'); const modalMovieGenre = document.getElementById('modal-movie-genre'); const modalMovieDuration = document.getElementById('modal-movie-duration'); const modalMovieDescription = document.getElementById('modal-movie-description'); const modalShowtimesList = document.getElementById('modal-showtimes-list'); const modalRatingSection = document.getElementById('modal-rating-section'); const modalStarsContainer = document.getElementById('modal-stars'); const modalRatingMessage = document.getElementById('modal-rating-message'); const seatMapContainer = document.getElementById('seat-map-container'); const seatGrid = document.getElementById('seat-grid'); const selectedSeatsInfo = document.getElementById('selected-seats-info'); const confirmSelectionButton = document.getElementById('confirm-selection-button'); const qrValidationModal = document.getElementById('qr-validation-modal'); const validationModalCloseBtn = document.getElementById('validation-modal-close-btn'); const validationModalCloseBtnSecondary = document.getElementById('validation-modal-close-btn-secondary'); const validationMovieTitle = document.getElementById('validation-movie-title'); const validationShowtime = document.getElementById('validation-showtime'); const validationSeats = document.getElementById('validation-seats'); const validationEmail = document.getElementById('validation-email'); const validationBookingId = document.getElementById('validation-booking-id'); const validationStatusElement = document.getElementById('validation-status'); const markUsedBtn = document.getElementById('mark-used-btn');

        // --- State ---
        let movies = []; let customerUsers = []; let vendors = []; /* New */ let userRatings = {}; let allBookings = []; let usedQrCodes = []; let authState = { isLoggedIn: false, user: null }; let editingIndex = null; let editingVendorIndex = null; /* New */ let selectedSeats = []; let currentAdminTab = 'admin-manage-movies'; let currentModalMovieId = null; let isShowingMyBookings = false; let html5QrCodeScanner = null;
        ...
        function renderUI() {
            loadAllBookingsFromStorage(); loadUsedQrCodes(); closeModal(); closeValidationModal();
            if (authState.isLoggedIn && authState.user) {
                hideElement(authView); showElement(appHeader);
                let userTypeDisplay = authState.user.type;
                if(authState.user.type === 'vendor' && authState.user.name) {
                    userTypeDisplay = `vendor (${authState.user.name})`; // Show vendor name
                }
                 loggedInUserInfo.textContent = `Logged in as: ${authState.user.email} (${userTypeDisplay})`;

                if (authState.user.type === 'admin') { hideElement(customerView); hideElement(vendorDashboardView); showElement(adminView); renderAdminContent(); }
                else if (authState.user.type === 'vendor') { hideElement(customerView); hideElement(adminView); showElement(vendorDashboardView); renderVendorDashboard(); }
                else { hideElement(adminView); hideElement(vendorDashboardView); showElement(customerView); renderCustomerContent(); }
            } else { hideElement(appHeader); hideElement(adminView); hideElement(customerView); hideElement(myBookingsView); hideElement(vendorDashboardView); showElement(authView); showCustomerLoginForm(); }
        }
        ...
        function renderAdminContent() {
             ...
             if (currentAdminTab === 'admin-manage-movies') { renderMovies(adminMovieListContainer, 'admin'); renderMovies(customerPreviewContainer, 'preview'); populateVendorDropdown('movie-vendor'); } // Populate dropdown here
             ...
             else if (currentAdminTab === 'admin-manage-vendors') { renderAdminVendorManagement(); } // Render vendor management UI
        }
        ...
        function handleMovieFormSubmit(event) {
            event.preventDefault(); const indexInput = document.getElementById('edit-index').value; const isEditing = indexInput !== ''; const index = isEditing ? parseInt(indexInput, 10) : -1;
            const movieData = {
                ...
                vendorName: movieVendorSelect.value || null, // Get selected vendor name
                movieId: isEditing && index >= 0 && index < movies.length ? movies[index].movieId : generateId(),
                ratings: isEditing && index >= 0 && index < movies.length ? movies[index].ratings : []
            };
            if (!movieData.title || !movieData.poster || !movieData.genre || !movieData.showtimes || isNaN(movieData.duration) || movieData.duration <= 0) { alert("Please fill all fields correctly."); return; }
            if (isEditing) { if (index >= 0 && index < movies.length) { movies[index] = movieData; } else { console.error("Invalid index during edit:", index); return; } } else { movies.push(movieData); }
        ...
        function showEditForm(index) {
            ...
            populateVendorDropdown('movie-vendor'); // Ensure dropdown is populated before setting value
            ...
            movieVendorSelect.value = movie.vendorName || ""; // Set dropdown selection
        ...
        function renderVendorList() {
            if (!adminVendorListContainer) return; adminVendorListContainer.innerHTML = ''; loadVendorsFromStorage();
            if (vendors.length === 0) { adminVendorListContainer.innerHTML = '<p class="text-gray-500">No vendors registered yet.</p>'; return; }
            vendors.forEach((vendor, index) => {
                const itemDiv = document.createElement('div'); itemDiv.className = 'vendor-list-item';
                itemDiv.innerHTML = `
                    ...
                    <div class="flex-shrink-0 flex gap-2">
                        <button class="btn btn-secondary btn-icon text-xs !py-1 !px-2" onclick="showEditVendorForm(${index})"><i class="fas fa-edit"></i> Edit</button>
                        <button class="btn btn-danger btn-icon text-xs !py-1 !px-2" onclick="deleteVendor(${index})"><i class="fas fa-trash"></i> Delete</button>
                    </div>
                `;
        ...
        function handleVendorFormSubmit(event) {
            event.preventDefault(); const indexInput = document.getElementById('edit-vendor-index').value; const isEditing = indexInput !== ''; const index = isEditing ? parseInt(indexInput, 10) : -1;
            const vendorData = {
                vendorId: isEditing && index >= 0 && index < vendors.length ? vendors[index].vendorId : generateId(),
            ...
            if (!vendorData.name || !vendorData.email || !vendorData.password) { alert("Please fill all vendor fields."); return; }
            // Basic email format check (optional but good)
            if (!/^\S+@\S+\.\S+$/.test(vendorData.email)) { alert("Please enter a valid email address."); return; }
            // Check for duplicate email (excluding self if editing)
            if (vendors.some((v, i) => v.email === vendorData.email && i !== index)) { alert("A vendor with this email already exists."); return; }

            if (isEditing) { if (index >= 0 && index < vendors.length) { vendors[index] = vendorData; } else { console.error("Invalid index during vendor edit:", index); return; } }
            else { vendors.push(vendorData); }
            saveVendorsToStorage(); renderVendorList(); resetVendorForm(); populateVendorDropdown('movie-vendor'); // Update movie form dropdown too
        }
        ...
        function deleteVendor(index) {
            ...
            if (confirm(`Delete vendor "${vendorName}"? Movies assigned to this vendor will be unassigned.`)) {
                vendors.splice(index, 1); saveVendorsToStorage();
                // Disassociate movies from the deleted vendor
                let moviesUpdated = false;
                movies.forEach(movie => { if (movie.vendorName === vendorName) { movie.vendorName = null; moviesUpdated = true; } });
                if (moviesUpdated) saveMoviesToStorage();
                renderVendorList(); resetVendorForm(); populateVendorDropdown('movie-vendor');
            }
        }
        function populateVendorDropdown(selectElementId) {
            ...
            select.innerHTML = '<option value="">-- None --</option>'; // Start fresh
            loadVendorsFromStorage(); // Ensure vendors are loaded
            ...
            select.value = currentVal; // Restore previous value if still valid
        }

        // --- Vendor Dashboard Rendering ---
        function renderVendorDashboard() {
            if (!vendorMovieListContainer || !noVendorMoviesMsg) return;
            vendorMovieListContainer.innerHTML = ''; // Clear previous
            if (!authState.isLoggedIn || authState.user.type !== 'vendor' || !authState.user.name) {
                noVendorMoviesMsg.textContent = 'Error: Invalid vendor session.'; showElement(noVendorMoviesMsg); return;
            }
            ...
            if (vendorMovies.length === 0) { noVendorMoviesMsg.textContent = `No movies assigned to vendor "${vendorName}".`; showElement(noVendorMoviesMsg); }
                ...
                vendorMovies.forEach((movie) => {
                    // Find original index if needed for actions later, otherwise null
                    const originalIndex = movies.findIndex(m => m.movieId === movie.movieId);
                    // Create movie card - specify 'vendor' view type to potentially customize actions later
                    const card = createMovieCard(movie, originalIndex, 'vendor_dashboard');
                    vendorMovieListContainer.appendChild(card);
                });
            }
            // Placeholder for potential vendor-specific analytics
            // renderVendorAnalytics(vendorName);
        }
        ...
        function createMovieCard(movie, index, viewType) {
            ...
            const title = document.createElement('h3'); title.className = 'text-lg font-semibold mb-1 text-gray-800 flex items-center'; // Flex for badge alignment
            title.textContent = movie.title;
            // Add Vendor Badge if applicable
            if (movie.vendorName) {
                const vendorBadge = document.createElement('span');
                vendorBadge.className = 'vendor-badge';
                vendorBadge.textContent = movie.vendorName;
                title.appendChild(vendorBadge); // Append badge next to title text
            }
            const avgRatingDiv = document.createElement('div'); avgRatingDiv.className = 'stars-container text-sm mb-2'; const { average, count } = calculateAverageRating(movie); renderStars(avgRatingDiv, average, count);
            const details = document.createElement('p'); details.className = 'text-sm text-gray-600 mb-1'; details.innerHTML = ` <i class="fas fa-tag mr-1 opacity-75"></i> ${movie.genre || 'N/A'} • <i class="fas fa-clock mr-1 opacity-75"></i> ${movie.duration || 'N/A'} min `;
            ...
            // --- Controls based on View Type ---
            if (viewType === 'admin') { // Admin sees full controls
            ...
            } else if (viewType === 'customer' || viewType === 'preview') { // Customer/Preview: Clickable for modal
                card.classList.add('cursor-pointer'); card.onclick = () => openModal(movie.movieId);
            } else if (viewType === 'vendor_dashboard') { // Vendor: View-only for now (could add vendor-specific actions later)
                 // No controls added for now, just displays info
            }
            return card;
        }

        // --- Movie List Rendering --- (No change needed)
        function renderMovies(container, viewType) { container.innerHTML = ''; let noMoviesMsg; if (viewType === 'admin') noMoviesMsg = noMoviesAdminMsg; else if (viewType === 'preview') noMoviesMsg = noMoviesPreviewMsg; else if (viewType === 'vendor_dashboard') noMoviesMsg = noVendorMoviesMsg; else noMoviesMsg = noMoviesCustomerMsg; if (movies.length === 0 && viewType !== 'vendor_dashboard') { if(noMoviesMsg) showElement(noMoviesMsg); return; } else { if(noMoviesMsg) hideElement(noMoviesMsg); } const moviesToRender = viewType === 'vendor_dashboard' ? movies.filter(m => m.vendorName === authState?.user?.name) : movies; if(moviesToRender.length === 0 && viewType === 'vendor_dashboard') { if(noMoviesMsg) {noMoviesMsg.textContent = `No movies assigned to vendor "${authState?.user?.name}".`; showElement(noMoviesMsg); } return; } moviesToRender.forEach((movie) => { const originalIndex = movies.findIndex(m => m.movieId === movie.movieId); const card = createMovieCard(movie, originalIndex, viewType); container.appendChild(card); }); }
        ...
        function handleAdminTabClick(event) {
            ...
            const validTargets = ['admin-manage-movies', 'admin-analytics', 'admin-bookings-section', 'admin-qr-scanner-section', 'admin-manage-vendors']; // Added vendors
            if (!targetId || targetId === currentAdminTab || !validTargets.includes(targetId)) { return; }
        ...
        function initializeApp() {
            ...
            loadMoviesFromStorage(); loadCustomersFromStorage(); loadVendorsFromStorage(); /* Load vendors */ loadUserRatingsFromStorage(); loadAuthFromStorage(); loadAllBookingsFromStorage(); loadUsedQrCodes();
            // Event listeners
            ...
            vendorLoginForm.addEventListener('submit', handleVendorLogin); /* Add vendor login handler */
            ...
            showVendorLoginButton.addEventListener('click', showVendorLoginForm); /* Add vendor button handler */
            ...
            vendorForm.addEventListener('submit', handleVendorFormSubmit); /* Add vendor form handler */
            vendorCancelEditButton.addEventListener('click', resetVendorForm); /* Add vendor cancel handler */
